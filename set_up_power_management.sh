...
echo "ACTION==\"change\", SUBSYSTEM==\"power_supply\", KERNEL==\"AC0\", ATTR{online}==\"1\", RUN+=\"/bin/sh -c '${ac_command[@]}'\"
ACTION==\"change\", SUBSYSTEM==\"power_supply\", KERNEL==\"AC0\", ATTR{online}==\"0\", RUN+=\"/bin/sh -c '${battery_command[@]}'\"" | sudo tee /etc/udev/rules.d/99-powertrigger.rules
